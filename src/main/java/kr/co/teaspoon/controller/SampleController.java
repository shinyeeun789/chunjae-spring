...
public class SampleController {
    ...
    @RequestMapping("main.do")                 // GET 방식, localhost:8081/pro03_war/sample/main.do
    public String sample(Locale locale, Model model) throws Exception {
        model.addAttribute("user", "김이름");
        return "/sample/main";
    }

    @GetMapping("get1.do")                     // GET 방식, <a href="/sample/get1.do?id=sye&pw=asdf"> GET 방식 </a>
    ...
    // post 방식이나 특정 템플릿을 써야만 기능이 제공됨
    @RequestMapping(value="get4.do", method=RequestMethod.GET)
    public String get4(People member, Model model) {
    ...
    // querystring 방식, pathstring 방식
    // GET 방식에서 주로 활용하며, springboot나 react에서 전달 가능한 방식
    ...
    public String post4(@RequestParam("id") String id, @RequestParam("pw") String pw, Model model) throws Exception {
        model.addAttribute("id", id);
        model.addAttribute("pw", pw);
        return "/sample/post3";
    }
    ...
    public String post4(People member, Model model) throws Exception {
        return "/sample/post5";                     // ${member.id}, ${member.pw}
    }
    ...
    // GET 방식에서 주로 활용하며, spring에서만 저장 목적의 전달만 가능한 방식임
